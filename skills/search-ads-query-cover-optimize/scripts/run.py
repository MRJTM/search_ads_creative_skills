#!/usr/bin/env python3
"""CLI wrapper for the cover optimize skill.

Reads a JSON payload from stdin, runs the async core function
``agent.skills_core.skill_cover_optimize`` and prints the result as
...
def main() -> int:
    ...
    try:
        ...
        from agent.models import CoverOptimizeInput
        from agent.skills_core import skill_cover_optimize

        result = asyncio.run(skill_cover_optimize(CoverOptimizeInput(**payload)))
