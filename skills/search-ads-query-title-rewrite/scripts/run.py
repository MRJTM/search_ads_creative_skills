#!/usr/bin/env python3
"""CLI wrapper for the title rewrite skill.

Reads a JSON payload from stdin, runs the async core function
``agent.skills_core.skill_title_rewrite`` and prints the result as
...
def main() -> int:
    ...
    try:
        ...
        from agent.models import TitleRewriteInput
        from agent.skills_core import skill_title_rewrite

        result = asyncio.run(skill_title_rewrite(TitleRewriteInput(**payload)))
